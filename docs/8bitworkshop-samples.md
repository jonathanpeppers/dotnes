...
>
> Existing dotnes samples: `hello`, `hellofs`, `staticsprite`, `movingsprite`, `attributetable`, `flicker`, `metasprites`, `music`, `lols`, `tint`, `scroll`, `rletitle`, `tileset1`, `sprites`, `metacursor`, `metatrigger`, `statusbar`, `vrambuffer`, `horizscroll`, `horizmask`, `fami`, `climber`, `nestedloop`
...
| `sbyte` (signed char) — negative constants, explicit narrowing and sign extension | climber, shoot2, siegegame |
...
- [x] **sbyte (signed char)** — Two's-complement storage with explicit truncation/sign extension and promoted signed comparisons. See the [numeric model](numeric-model.md) for supported ranges and `int` limitations.
...
- [x] **Bit shifts (Shr/Shl)** — Unsigned right shifts use LSR/ROR; signed right shifts preserve the sign. Left shifts use ASL/ROL. See the [numeric model](numeric-model.md) for signed-count limitations.
