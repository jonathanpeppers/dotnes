...
| **Deterministic test data** | Tests should not depend on system locale, timezone, current date, filesystem ordering, or random seeds. Use explicit invariant formatting and fixed inputs. ROM output must be fully deterministic. |
| **Exercise concurrency contracts** | When code introduces shared caches, cancellation, or parallel work, test concurrent calls and cancellation paths rather than only the single-threaded happy path. |
