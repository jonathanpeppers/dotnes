...
| **Local variable allocation** | Local variables are allocated starting at `$0325` (`LocalStackBase` in `NESConstants.cs`). This is NES RAM, not zero page. Each new local gets a RAM slot (1 byte for `byte`, 2 bytes for `ushort`/`short`). If two variables share a slot when they shouldn't, they'll overwrite each other. Verify allocation doesn't overlap with NESLib's reserved RAM usage. |
